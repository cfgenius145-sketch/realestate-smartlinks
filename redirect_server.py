# redirect_server.py — SmartLinks Backend
# Fixes:
# - Per-user isolation using an owner token (no login)
# - 3-link free cap per owner
# - Clean PDF & CSV; Pacific time
...
# ===== Owner key (token-based, fallback to IP) =====
_ip_re = re.compile(r'^\s*([^,\s]+)')
def get_owner_key(request: Request) -> str:
    # Prefer explicit token from client (Streamlit passes this)
    tok = request.headers.get("X-Owner-Token")
    if tok:
        return f"tok:{tok.strip()[:64]}"
...
# ===== Limits =====
FREE_LIMIT_PER_OWNER = int(os.getenv("FREE_LIMIT_PER_IP", "3"))  # keep the env var name you already set
...
async def create_link(data: CreateLinkIn, request: Request):
    ...
    if (c.fetchone()[0] or 0) >= FREE_LIMIT_PER_OWNER:
...
def report_pdf(short_code: str, request: Request):
    ...
    total, uniq = stats["total"], stats["unique_visitors"]
    scans = total
